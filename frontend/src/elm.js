...
var $author$project$Main$WindowFocusChanged = function (a) {
	return {$: 'WindowFocusChanged', a: a};
};
...
var $author$project$Main$windowFocusChanged = _Platform_incomingPort(
	'windowFocusChanged',
	...
	return $elm$core$Platform$Sub$batch(
		_List_fromArray(
			[
				...
				$author$project$Main$windowFocusChanged($author$project$Main$WindowFocusChanged)
			]));
};
...
var $author$project$Main$resetKeyStatuses = function (model) {
	return _Utils_update(
		model,
...
var $author$project$Main$undoMove = F3(
	function (model, cmds, command) {
		...
		var debugString = 'Moving ' + ($elm$core$String$fromInt(
			$elm$core$List$length(filesToMove)) + (' to ' + destination));
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					debug: A2($elm$core$List$cons, debugString, model.debug),
					isUndoing: true
				}),
			A2(
				$elm$core$List$cons,
...
var $author$project$Main$processMainShortcuts = F3(
	function (model, target, event) {
		if (event.ctrlKey || event.metaKey) {
			...
			while (true) {
				if (_v0.b) {
					if (_v0.a.$ === 'A') {
						var _v1 = _v0.a;
						return _Utils_Tuple2(
							$author$project$Main$resetKeyStatuses(
								A3($author$project$Main$changeAllFileStatus, model, target, $author$project$File$Unselected)),
							$elm$core$Platform$Cmd$none);
					} else {
						break _v0$5;
					}
					...
					switch (_v0.a.$) {
						case 'A':
							...
							return _Utils_Tuple2(
								$author$project$Main$resetKeyStatuses(
									A3($author$project$Main$changeAllFileStatus, model, target, $author$project$File$Selected)),
								$elm$core$Platform$Cmd$none);
						case 'R':
							var _v3 = _v0.a;
			...
			return _Utils_Tuple2(
				$author$project$Main$resetKeyStatuses(model),
				$elm$core$Platform$Cmd$none);
		} else {
			if (event.altKey) {
		...
		switch (msg.$) {
			...
			case 'UserClickedSourceDirectory':
				...
				var newSourcePath = function () {
					var _v5 = _Utils_Tuple2(isAbsolute, file.name);
					if (_v5.a) {
						return _Utils_eq(file.parentPath, $author$project$Main$unixPathSep) ? _Utils_ap(model.pathSeparator, file.name) : _Utils_ap(
							file.parentPath,
							_Utils_ap(model.pathSeparator, file.name));
			...
			case 'UserSubmittedFilename':
				...
				var isConflicting = A2(
					...
					model.sourceFiles);
				var _v6 = _Utils_Tuple3(model.editedFile, isConflicting, isNameEmpty);
				_v6$2:
				while (true) {
					if (_v6.a.$ === 'Just') {
						if (_v6.b) {
