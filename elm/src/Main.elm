...
changeAllFileStatus : Model -> Target -> FileStatus -> Model
    ...
    if event.ctrlKey || event.metaKey then
        ...
        case ( event.keyCode, event.shiftKey ) of
            ...
            ( Key.Up, True ) ->
                case target of
                    Source ->
                        ( { model | sourceFiles = extendSelectionToPrevious model.sourceFiles }
                            |> resetKeyStatuses
                        , Cmd.none
                        )

                    Destination ->
                        ( { model | destinationFiles = extendSelectionToPrevious model.destinationFiles }
                            |> resetKeyStatuses
                        , Cmd.none
                        )
            ...
            ( Key.Down, True ) ->
                case target of
                    Source ->
                        ( { model | sourceFiles = extendSelectionToNext model.sourceFiles }
                            |> resetKeyStatuses
                        , Cmd.none
                        )

                    Destination ->
                        ( { model | destinationFiles = extendSelectionToNext model.destinationFiles }
                            |> resetKeyStatuses
                        , Cmd.none
                        )
